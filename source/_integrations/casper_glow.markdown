...
- **Paused**
  - **Description**: Indicates whether the dimming sequence is currently paused.

- **Charging**
  - **Description**: Indicates whether the Casper Glow is currently plugged into a charger.
  - **Category**: Diagnostic
  - **Remarks**: Disabled by default. Enable it from the entity registry if you want to use it in automations or dashboards.

### Buttons

- **Pause dimming**
  - **Description**: Pauses an active dimming sequence. The light stays at its current brightness until you resume.

- **Resume dimming**
  - **Description**: Resumes a paused dimming sequence, continuing from where it left off.
...
- **Dimming time**
  - **Description**: Configures how long the dimming sequence takes to complete.
...
- **Battery**
  - **Description**: Reports the remaining battery level of the Glow light as a percentage.
  - **Category**: Diagnostic

- **Remaining dimming time**
  - **Description**: Reports the number of minutes remaining in the current dimming sequence.
  - **Remarks**: Disabled by default. Enable it from the entity registry if you want to use it in automations or dashboards.

## Actions

The integration provides the following actions for controlling the dimming sequence.

### Action: Pause

The `casper_glow.pause` action pauses an active dimming sequence on the targeted Glow light. The light stays at its current brightness level until you resume.

- **Target**: A Casper Glow light entity.

### Action: Resume

The `casper_glow.resume` action resumes a previously paused dimming sequence, continuing from where it left off.
...
actions:
  ...
  - wait_for_trigger:
      - trigger: template
        value_template: >-
          {{ states('sensor.jar_remaining_dimming_time') | int(default=30) < 10 }}
    timeout:
      minutes: 30
...
- The dimming time can only be set to one of five preset durations (15, 30, 45, 60, or 90 minutes).
- Changing the **Dimming time** selector while the light is on does not affect the current session. The new duration takes effect the next time the light is turned on.
